(() => {
    ...
    window.onload = () => {
        ...
        let data_formatter = (data) => {
            ...
            let links = ''
            if (data.options)
                links += `<a href="${ data.options }" target="_blank" class="options">options</a>`

            if (data.home)
                links += `<a href="${ data.home }" target="_blank" class="home">home</a>`
            ...
            <div class="extension ${ data.install_type }" id="${data.id}">
                ...
                ${ data.description } >> 
                ${ data.type } >> 
                <input class="checkbox" type="checkbox" ${ data.is_enabled ? 'checked': '' }>

                <hr>
                ${ links }
                <a href="${ data.details }" target="_blank" class="details">details</a>
            ...
            if (all_extensions != null && all_extensions.length > 0) {
                ...
                all_extensions.forEach(extension => {
                    ...
                    if (extension.type === "extension")
                        _extensions_innerHTML += data_formatter(extension_to_data(extension))
                    else
                    if (extension.type === "theme")
                        _themes_innerHTML += data_formatter(extension_to_data(extension))
                    else
                        _apps_innerHTML += data_formatter(extension_to_data(extension))
